...
import edu.upenn.cis455.configuration.ImageResponse;
 ...
 * 
 */
public class ImageDataAccessor {
	...
	public static synchronized void put(ImageEntry obj) {
		PrimaryIndex<String, ImageEntry> pIndex = store.getPrimaryIndex(String.class, ImageEntry.class);
		System.out.print("Putting in BDB & printing count" + BDBInsert++);		
		pIndex.put(obj);
		environment.sync();
	}
	
	public static synchronized void putAll(ArrayList<ImageEntry> obj) {
		
		if(obj == null || obj.size() == 0){
			System.out.println("Null Objects recieved in PUTALL ********************************");
			return;
		}
		...
		for(ImageEntry entry : obj){
			pIndex.put(entry);
		}
		environment.sync();
	}
	
	public static synchronized ImageResponse getResults(String query, int top){
		
		// Return only top 2000 results sorted by the score value
		PrimaryIndex<String, ImageEntry> pIndex = store.getPrimaryIndex(String.class, ImageEntry.class);
		SecondaryIndex<String, String, ImageEntry> scoreIndex = store.getSecondaryIndex(pIndex, String.class, "tfIdf");
		...
		EntityCursor<ImageEntry> cursor = scoreIndex.entities(query, true, String.valueOf(stop), false);
		HashMap<String, ImageEntry> results = new HashMap<String, ImageEntry>();
		ImageResponse response = new ImageResponse();
		try {
			
			int count = 1;
			ImageEntry row = cursor.last();
			if(row == null){
				return response;
			}
