	...
	public static HashMap<String, String> makehead(ArrayList<String> head) {
		...
		for (String request : head) {
			...
			if (headerKeyValue.length > 2) {
				...
				// CASE handling when multiple headers have same keys, we append
				// all the values by comma separation
				if ((header.containsKey(key))) {
					...
					mostrecentKey = key;
					header.put(key, value); // check if we want to handle upper
											// case or not // check multiple
											// same keys
				}
				...
				if (header.containsKey(mostrecentKey) && Character.isWhitespace(headerKeyValue[0].charAt(0))) {
					...
					header.put(mostrecentKey, mostrecentValue + " " + headerKeyValue[0].trim()); // i
																									// think
																									// i
																									// missed
																									// the
